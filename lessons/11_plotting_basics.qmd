...
Let’s try to have both `celltype` and `genotype` represented on the plot. We can assign the `celltype` column to the shape argument in `scatterplot()`, so each celltype is plotted with a different shaped data point.
...
No, they don’t. We can, however, change both the x-axis labels and size labels with the `plt.xlabel()` functions from `matplotlib`. Since we will be adding this layer “on top” of, or after, theme_bw(), any features we change will override what is set by the `sns.set_style()` layer.
...
If you wanted to save this plot, you can use the `savefig()` function from `matplotlib` and specify the file name and format you want to save it in. By default this function will save the last plot that was generated, so make sure to call `savefig()` after you have generated the plot you want to save. For example, to save the plot as a PNG file, you can use:

```{python}
#| label: save_plot
#| eval: false
# Save the plot as a PNG file
plt.savefig("figures/scatterplot.png", format = "png")
``` 
