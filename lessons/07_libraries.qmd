...
Now that we have an understanding of what libraries are, we can talk about how to install them. There are many different ways to install packages in Python, but two of the more common means are: 

- Installing from the Anaconda Navigator
- Installing from the command line using `pip` or `conda`.

For this workshop, we will use the Anaconda Navigator to install packages. The Anaconda Navigator is a graphical user interface that allows you to manage your Python environments and packages without needing to use the command line.

### Create a new environment

Generally, the recommendation is to avoid installing backages on `base (root)`. So instead we are going to create a new environment. These are like separate "workspaces" where you can install packages on one environment without affecting the other environments. This can be useful for keeping your projects organized and be consistent with package versions across projects.

To begin, let's first create a new environment called `intro_python`. Navigate to the "Environments" tab in the Anaconda Navigator and click on the "Create" button. In the pop-up window, enter `intro_python` as the name of the environment and select Python 3.10.12 as the version. Then click on the "Create" button to create the environment.

::: {#fig-create_env .fig}
![](../img/anaconda_create_intro_python.png){width="50%"}

Pop-up window for creating a new environment in Anaconda Navigator, where the name of the environment is set to `intro_python` and the Python version is set to 3.10.
:::
...
First, change the dropdown menu to either "All" or "Not installed" instead of "Installed". Next, in the search bar, type `numpy` and press enter. You should see the `numpy` package listed in the search results. There many be several packages that match your search as all potential search matches are listed in alphabetical order. You may have to scroll until you see the `numpy` package. 
...
First, we will open our Jupyter Lab notebook and make sure that we are using the `intro_python` environment by setting our "kernel" to `intro_python`. Kernels are another way to refer to a Python environment in Jupyter Lab.

Navigate to the "Kernel" tab in the top menu and select "Change Kernel". 

::: {#fig-change_kernel .fig}
![](../img/anaconda_change_kernel.png){width="50%"}

Jupyter Lab menu option for changing the kernel to `intro_python` in order to use a different Python environment.    
:::

::: {.callout-note collapse="true"}
# Alternative way to change kernel
To check that we are using the correct kernel, we can look at the top right corner of our Jupyter Lab notebook. It should say `Python 3 (base)` or something similar. If it does not, you can click on the kernel name and select `intro_python` from the dropdown menu to switch to the correct environment.
:::

Then select `intro_python` from the dropdown menu to switch to the correct environment.

::: {#fig-change_kernel .fig}
![](../img/anaconda_select_kernel.png){width="50%"}

Dropdown menu for changing the kernel to `intro_python` to change the kernel to a different Python environment.
:::
...
This way we do not have to import the entire `sklearn` library and can just import the specific function that we need.
