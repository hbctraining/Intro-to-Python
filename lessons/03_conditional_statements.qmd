...
In this lesson, you will learn how to express `True`/`False` logic in Python so your scripts can adapt to different scenarios and datasets instead of inflexibility repeating the exact steps.
...
`if` statements are not exclusively numerical comparisons, we can also check if a string is equal `==` to another string, if a list contains a certain value, and so on. So we can also write:
...
## Is this intented to be an incorrect example? If so, we should add a callout to explain the error.
```{python}
#| label: elif_example
name = "Bob"
if name == "Alice":
