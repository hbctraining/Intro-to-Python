...
Sometimes you may want to have a "catch-all" block of code that executes when the `if` statement is not met. This is where `else` comes in! We can build upon the `my_name` example from above and add an `else` statement to handle the case where `name != "Anja"`:

```{python}
#| label: if_else_example
# Set the string "Noor" to the variable called my_name
my_name = "Noor"
...
Even though `x` is greater than 10, the first condition that is `True` is `x > 5`, so it will execute that block of code and skip the rest.
...
The `not` operator allows us to check if a condition is `False` and returns a `True`, or vice versa. For example, one way we could check if `x` is not greater than 10 would be to write: 
