...
You may notice that there are many functions that follow the structure `__method__` when we view the output for `dir()`. These are called **dunder methods** (short for “double underscore”) and they are special methods . They are typically used to define the behavior of data structures in certain scenarious. 
