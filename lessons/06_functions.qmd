...
A key feature of Python is functions. Functions are “self contained” modules of code that accomplish a specific task. Functions usually receive some sort of data structure (value, vector, dataframe etc.), process it, and return a result.
...
Lists are not the only data structure that has inate methods. For example, strings have a method called `upper()`, which converts all characters in the string to uppercase:
...
When you **define the function** you will need to provide the **list of arguments** required (inputs and/or options to modify behaviour of the function). The argument(s) can be any type of object (like a scalar, a matrix, a dataframe, a vector, a logical, etc), and it’s not necessary to define the type of object.
...
Now, we can use this function like any other base R functions. We first type out the name of the function, add the parentheses and write we provide a numeric value `x` inside the parentheses:
...
6. **Custom Functions** - Create a function `temp_conv()`, to convert the temperature in Fahrenheit (input) to the temperature in Kelvin (output). Let’s perform a two-step calculation: 
