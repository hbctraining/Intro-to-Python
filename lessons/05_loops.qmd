...
A common use of `for` loops is to iterate over a sequence of numbers. We can use the `range()` function to generate a sequence of numbers that we can iterate over. The `range()` function takes three arguments: `start`, `stop` and `step`. The `start` argument is the number that the sequence will start from, the `stop` argument is the number that the sequence will stop at (but not include) and the `step` argument is the amount by which the sequence will increment. If you do not provide the `step` argument, the default is 1.
...
Like `for` loops, `while` loops will to continue to execute a block of code as long as a certain condition is true. 
...
*Hint: You will likely want to use a modulo operator, `%`. Modulo operators tell you the remainder following division. For example, 4 mod 2 would return 0 because the remainder of 4 divided by 2 is zero, while 5 mod 2 would return 1 because the remainder of 5 divided by 2 is 1.*
