...
We now created a variable `organism` for the `for` loop. This is a variable that takes the value of each element in the `species` list as we iterate through it. So, in the first iteration of the loop, `organism` will be equal to `"ecoli"`, in the second iteration, `organism` will be equal to `"human"`, and so on.
